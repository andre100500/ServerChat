...
namespace ChatClient.ViewModels
{
    public class ChatViewModel : ViewModel
    {
        ...
        private UserEnum _userMode;
        public UserEnum UserMode
        {
        ...
        public  ICommand ConnectCommand => _connectCommand ?? (_connectCommand = new Command((e) => Connect()));
        ...
        public ICommand LoginCommand
        {
            get
            {
                return _loginCommand ?? (_loginCommand =
                    new Command((e) => Login(), (o) => CanLogin()));
            }
        }
        ...
        private async Task<bool> Login()
        {
            try
            {
                ...
                if (users != null)
                {
                    users.ForEach(u => Participants.Add(new Participant { Name = u.Name, Photo = u.Img }));
                    UserMode = UserEnum.Chat;
                    IsLoggedIn = true;
                    return true;
                }
                else
                {
                    dialogService.ShowNotify("Username is already in use");
                    return false;
                }
        ...
        public ICommand SendImageMessageCommand
        {
            get
            {
                return _sendImageMessageCommand ?? (_sendImageMessageCommand =
                    new CommandAsync(() => SendImageMessage(), (o) => CanSendImageMessage()));
            }
        }

        private async Task<bool> SendImageMessage()
        {
            var pic = dialogService.OpenFile();
        ...
        public ICommand SelectProfilePicCommand
        {
            get
            {
                return _selectProfilePicCommand ?? (_selectProfilePicCommand =
                    new Command((o) => SelectProfilePic()));
            }
        }
        ...
        private void SelectProfilePic()
        {
            ...
            if (!string.IsNullOrEmpty(pic))
            {
                ...
                {
                    dialogService.ShowNotify($"Image size should be {MAX_IMAGE_WIDTH} x {MAX_IMAGE_HEIGHT} or less.");
                    return;
                }
                ProfilePic = pic;
            }
        ...
        public ICommand OpenImageCommand
        {
            get
            {
                return _openImageCommand ?? (_openImageCommand =
                    new Command<ChatMessage>((m) => OpenImage(m)));
            }
        }
        ...
        public ChatViewModel(IChatService chatSvc, IDialogService diagSvc)
        {
